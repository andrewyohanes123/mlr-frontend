...
class API {
  ...
  constructor(host: string, port = 80) {
    ...
    this._req.interceptors.request.use((config) => {
      config.headers = {
        "x-access-token": `${localStorage.getItem("app_accessToken")}`,
        "x-refresh-token": `${localStorage.getItem("app_refreshToken")}`,
      };
      console.log(config);
      return config;
    });
  ...
  async update(entity: string, id: string, data: string) {
    const res = await this._req.put(
      `${this._baseURL}/apis/${entity}/${id}`,
      data
    );
    return res.data.data;
  }
  ...
  async login(username: string, password: string) {
    const res = await this._req.post(`${this._baseURL}/apis/auth/`, {
      username,
      password,
    });
  ...
  async rawGet<T>(
    ...
    return this._req.get<any, AxiosResponse<T, any>>(`${this._baseURL}/${parent}/${endpoint}`, option);
  }

  async rawPost(endpoint: string, data: any, parent: string = "apis") {
  ...
  async rawPut(endpoint: string, data: any, parent: string = "apis") {
    const res = await this._req.put(
      `${this._baseURL}/${parent}/${endpoint}`,
      data
    );
